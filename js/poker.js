...
(() => {
    ...
    class Load {
        ...
        createMessageBtn() {
            ...
            messageBtn.addEventListener('click', () => {
                const player0_message = document.querySelector('.player0_message');
                const player1_message = document.querySelector('.player1_message');
                const player2_message = document.querySelector('.player2_message');
                const player3_message = document.querySelector('.player3_message');
        ...
        // ロード時、ルール説明ボタンを表示
        createRuleBtn() {
            const rule_container = document.querySelector('.rule_container');
    ...
    class Message {
        ...
        handCondition(handListNumber, playerNumber, bluff = false // ブラフはカード補充後の手札がノーハンドの場合のみ、一定の確率にもとづいて適用する
        ) {
            // ノーハンド
            const high_card = [
                'う～ん・・・',
            ...
            // プレイヤー手札にもとづいてセリフを表示
            for (let checkHandListNum = 0; checkHandListNum < message_handLists.length; checkHandListNum++) {
                // ブラフを実行するかどうか決める。
        ...
        playerDecide(playerNumber, selectedCardsCount) {
            let messages = [];
        ...
        // メインプレイヤーのセリフを表示
        predict_subPlayers_handList(subPlayerNames) {
            const messages = [
                `は自信がありそうだ・・・。`,
                `は微妙な感じがする・・・。`,
                `は自信がなさそうだ・・・。`,
        ...
        before_Open_PlayersHand() {
            ...
            let messagesIndex = [0, 0, 0];
            for (let i = 1; i < this.players.length; i++) {
                console.log(`this.players[${i}].handListNumber: ${this.players[i].handListNumber}`, `this.players[${i}].bluff: ${this.players[i].bluff}`);
                ...
                this.animation([`${messages[messagesIndex[i - 1]]}`], this.messagesArea[i]);
            }
        ...
        show_playerHand(handList, strongNumber, playerNumber) {
            if (this.messagesArea[playerNumber] === null) {
                throw new Error(`Error! this.messagesArea[${playerNumber}] === null`);
            }
            let strongCardName = '';
            ...
            if (strongNumber === 1 ||
                ...
                strongNumber === 12 ||
                strongNumber === 13) {
                this.animation([`${handList} の ${strongCardName} ！`], this.messagesArea[playerNumber]);
            }
            else {
                this.animation([`${handList} の ${strongNumber} ！`], this.messagesArea[playerNumber]);
            }
        }
        // 結果リストを表示する直前のセリフを表示
        before_resultList() {
        ...
        // 順位に応じたセリフを表示
        playerRank(rank, playerNumber) {
            if (this.messagesArea[playerNumber] === null) {
                throw new Error(`Error! this.messagesArea[${playerNumber}] === null`);
            }
            if (rank === 1) {
                this.animation([`よし！ ${rank} 位！！！`], this.messagesArea[playerNumber]);
            }
            else if (rank === 2) {
                this.animation([`${rank} 位だ！`], this.messagesArea[playerNumber]);
            }
            else if (rank === 3) {
                this.animation([`${rank} 位か～。`], this.messagesArea[playerNumber]);
            }
            else {
                this.animation([`${rank} 位……。`], this.messagesArea[playerNumber]);
            }
        }
    }
    // よく使うメソッドはこちらにまとめておく。
    class Tool {
        ...
        // }
        // this.renderHand(this._player0_handArea, 0); 「!」はつけない。
        createSingleLineBtn(area, cssName, text) {
        ...
        createDoubleLineBtn(area, cssName, text) {
            const btn = document.createElement('div');
    ...
    class Hand_list {
        ...
        checkHand_And_rearrangesCards(playerNumber) {
        ...
        showResultHandList(handListName, cssName, playerNumber) {
        ...
        check_sameNumberCount(verifyPlayerHands, matchNumber, playerNumber, checkHandListName = 'other') {
            ...
            if (!(checkHandListName === 'two_pair')) {
                ...
                if (checkHandListName === 'a_full_house') {
                    if (matchNumber === 3) {
                        const placePlayerHand_uncommon = this._poker.players[playerNumber].hand.filter((card) => !(card.number === matchNumber_Hand));
                        const placePlayerHand_common = this._poker.players[playerNumber].hand.filter((card) => card.number === matchNumber_Hand);
                        this.placePlayerHand = [];
    ...
    class Poker {
        constructor() {
            ...
            setTimeout(() => {
                // ストックの位置にボタンを配置
                this._tool.createDoubleLineBtn(document.querySelector('.stock'), 'startBtn', 'ポーカー<br>開 始！');
        ...
        createTrump() {
            ...
            // 「1」の strongNumber を 1 に修正する必要がある。
            let carsCount = 1;
            this._suits.forEach((suit, index) => {
                for (let i = 1; i <= 13; i++) {
                    if (i === 1) {
        ...
        createPlayers() {
            ...
            const player0_HandNext_LeftPoint = 112.5; // card width 100px + margin right 12.5px
            const player0_HandNext_TopPoint = 0; // すべて同じ座標
            const player0_DiscardNext_LeftPoint = 112.5; // card width 100px + margin right 12.5px
            const player0_DiscardNext_TopPoint = 0; // すべて同じ座標
            const player1_HandNext_LeftPoint = 0; // すべて同じ座標
            const player1_HandNext_TopPoint = 110.0; // card height 100px + margin bottom 10px
            const player1_DiscardNext_LeftPoint = 0; // すべて同じ座標
            const player1_DiscardNext_TopPoint = 110; // card height 100px + margin bottom 10px
            const player2_HandNext_LeftPoint = 112.5; // card width 100px + margin right 12.5px
            const player2_HandNext_TopPoint = 0; // すべて同じ座標
            const player2_DiscardNext_LeftPoint = 112.5; // card width 100px + margin right 12.5px
            const player2_DiscardNext_TopPoint = 0; // すべて同じ座標
            const player3_HandNext_LeftPoint = 0; // すべて同じ座標
            const player3_HandNext_TopPoint = 110; // card height 100px + margin bottom 10px
            const player3_DiscardNext_LeftPoint = 0; // すべて同じ座標
            const player3_DiscardNext_TopPoint = 110; // card height 100px + margin bottom 10px
        ...
        getPoint(area, playerNumber, nextLeft, nextTop) {
        ...
        renderPlayerName(area, index) {
            const nameArea = document.querySelector(area);
        ...
        stockAnimation() {
            ...
            for (let listNum = 0; listNum < 5; listNum++) {
                for (let playerNum = 0; playerNum < this._players.length; playerNum++) {
                    const handLists = document.querySelectorAll(`.player${playerNum}_hand>li`);
        ...
        // プレイヤー 手札 捨て札を初期化する（透明）
        initializationPlayerArea(playerArea) {
        ...
        // 捨て札を描画
        renderDiscard(discardArea, index) {
            while (discardArea.firstChild) {
                discardArea.firstChild.remove();
            }
            this._players[index].discard.forEach((discard) => {
                const li = document.createElement('li');
                const img = document.createElement('img');
                // 捨て札は全員 表示してもよいと思うけど、
        ...
        // 手札を描画
        renderHand(handArea, index) {
            while (handArea.firstChild) {
                handArea.firstChild.remove();
            }
            this._players[index].hand.forEach((hand) => {
                const li = document.createElement('li');
                const img = document.createElement('img');
        ...
        createExchangeBtn() {
            ...
            exchangeBtn.addEventListener('click', () => {
                ...
                // カードのクリックを禁止する。
                const selectedCards = document.querySelectorAll(`.player${this.playerTurn}_hand>li`);
                ...
                if (this._selectedCardsCount === 0) {
                    ...
                    setTimeout(() => {
                        ...
                        this._players[this.playerTurn].hand.forEach((card) => {
                            card.selected = false;
                        });
                        // 現在の手札と役が一致しているかどうかチェックおよび内部的に役に対応した並び替えを行う
        ...
        discard_SelectedCards_Animation() {
            ...
            // 型引数を指定することで、メソッドの返り値が HTMLElement | null 型となり、
            // style プロパティにもアクセスできるようになります。
            const playerLists = document.querySelectorAll(`.player${this.playerTurn}_hand>li`);
        ...
        subPlayerThought() {
            ...
            setTimeout(() => {
                const playerCards = document.querySelectorAll(`.player${this.playerTurn}_hand>li`);
        ...
        // 絵柄の一致数に基づいて意思決定する
        decisionMaking_common_suit(bool_common_suit) {
            const playerCards = document.querySelectorAll(`.player${this.playerTurn}_hand>li`);
            // 絵柄ごとの保有枚数の把握
            let spadeCount = 0;
            let heartCount = 0;
        ...
        decisionMaking_straight_number(bool_straight_number) {
            const playerCards = document.querySelectorAll(`.player${this.playerTurn}_hand>li`);
        ...
        nextTurn() {
            ...
            // メインプレイヤーの挙動
                ...
                // カードをクリックできるようにする。
                const selectedCards = document.querySelectorAll('.player0_hand>li');
                selectedCards.forEach((card) => {
        ...
        open_PlayersHand() {
            ...
            this._players.forEach((player) => {
                for (let i = 1; i < 5; i++) {
                    if (player.rank === i) {
                        const rank_rank = document.querySelector(`.rank${i}>.rank_rank`);
                        rank_rank.textContent = String(player.rank) + ' 位';
                        const rank_image = document.querySelector(`.rank${i}>.rank_image>img`);
                        rank_image.src = player.image;
                        const rank_name = document.querySelector(`.rank${i}>.rank_name`);
                        rank_name.textContent = player.name;
                        // innerHTML を使うのはロイヤルストレートフラッシュなどの場合、改行が必要になるため。
                        const rank_handlist = document.querySelector(`.rank${i}>.rank_handlist`);
                        rank_handlist.innerHTML = player.strong.handList;
                        // どの役であろうと、最初の比較対象となるカードは一番右のカードになる。
                        const rank_high_number_image = document.querySelector(`.rank${i}>.rank_high_number>img`);
                        rank_high_number_image.src = player.hand[4].url;
                        const rank_compare_number_image = document.querySelector(`.rank${i}>.rank_compare_number`);
            ...
            setTimeout(() => {
                for (let i = 1; i < this._players.length; i++) {
                    const playerHandCardsImage = document.querySelectorAll(`.player${i}_hand>li>img`);
            ...
            setTimeout(() => {
                ...
                this._message.show_playerHand(this._players[1].strong.handList, this._players[1].hand[4].number, 1);
            }, 5000);
            // 7 秒後
            setTimeout(() => {
                this._message.hideMessage();
                this._message.openMessage(2);
                this._message.show_playerHand(this._players[2].strong.handList, this._players[2].hand[4].number, 2);
            }, 7000);
            // 9 秒後
            setTimeout(() => {
                this._message.hideMessage();
                this._message.openMessage(3);
                this._message.show_playerHand(this._players[3].strong.handList, this._players[3].hand[4].number, 3);
            }, 9000);
            // 11 秒後
            setTimeout(() => {
                this._message.hideMessage();
                this._message.openMessage(0);
                this._message.show_playerHand(this._players[0].strong.handList, this._players[0].hand[4].number, 0);
            }, 11000);
            // 13 秒後
            // 結果リストを表示する直前のセリフを表示
            setTimeout(() => {
            ...
            // カードを配り直すボタンを表示
            setTimeout(() => {
                this._tool.createDoubleLineBtn(this.handAreas[0], 'backToStartBtn', 'カードを<br>配り直す');
